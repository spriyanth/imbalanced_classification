...
model_WBAG = BaggingClassifier(base_estimator = SVC(kernel = 'rbf', class_weight = {0:1, 1:2.5}, probability = True, random_state = seed1),
                               n_estimators = 50, max_samples = 0.8, bootstrap = True, n_jobs = -1, random_state = seed1)

model_WXGB = XGBClassifier(booster = 'gbtree', n_estimators = 200, max_depth = 2, min_child_weight = 2, scale_pos_weight = 7, random_state = seed1, early_stopping_rounds = 10)

############### DEFINE SAMPLING METHODS ###############

ROS1 = RandomOverSampler(sampling_strategy = 0.4, random_state = seed1)
SMOTE1 = SMOTE(sampling_strategy = 0.4, k_neighbors = 6, random_state = seed1)
ADASYN1 = ADASYN(sampling_strategy = 0.3, n_neighbors = 6, random_state = seed1)

RUS1 = RandomUnderSampler(sampling_strategy = 0.5, random_state = seed1)
ENN1 = EditedNearestNeighbours(sampling_strategy = 'auto', n_neighbors = 7, kind_sel = 'all')
NCL1 = NeighbourhoodCleaningRule(sampling_strategy = 'auto', n_neighbors = 7, threshold_cleaning = 0.5, n_jobs = -1)

ROS2 = RandomOverSampler(sampling_strategy = 0.6, random_state = seed1)
SMOTE2 = SMOTE(sampling_strategy = 0.4, k_neighbors = 5, random_state = seed1)
ADASYN2 = ADASYN(sampling_strategy = 0.3, n_neighbors = 5, random_state = seed1)

RUS2 = RandomUnderSampler(sampling_strategy = 0.6, random_state = seed1)
ENN2 = EditedNearestNeighbours(sampling_strategy = 'auto', n_neighbors = 2, kind_sel = 'all')
NCL2 = NeighbourhoodCleaningRule(sampling_strategy = 'auto', n_neighbors = 4, threshold_cleaning = 0.5, n_jobs = -1)
