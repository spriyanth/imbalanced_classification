...
model_XGB = XGBClassifier(booster = 'gbtree', n_estimators = 200, depth_range = 3, min_child_weight = 1, random_state = seed1, early_stopping_rounds = 10)

model_WBAG = BaggingClassifier(base_estimator = SVC(kernel = 'rbf', class_weight = {0:1, 1:2.5}, probability = True, random_state = seed1),
                               n_estimators = 50, max_samples = 0.8, bootstrap = True, n_jobs = -1, random_state = seed1)

model_WXGB = XGBClassifier(booster = 'gbtree', n_estimators = 200, max_depth = 2, min_child_weight = 1, scale_pos_weight = 2, random_state = seed1, early_stopping_rounds = 10)

############### DEFINE SAMPLING METHODS ###############

ROS1 = RandomOverSampler(sampling_strategy = 0.9, random_state = seed1)
SMOTE1 = SMOTE(sampling_strategy = 1.0, k_neighbors = 3, random_state = seed1)
ADASYN1 = ADASYN(sampling_strategy = 0.8, n_neighbors = 5, random_state = seed1)

RUS1 = RandomUnderSampler(sampling_strategy = 1.0, random_state = seed1)
ENN1 = EditedNearestNeighbours(sampling_strategy = 'auto', n_neighbors = 4, kind_sel = 'all')
...
SMOTE2 = SMOTE(sampling_strategy = 0.9, k_neighbors = 4, random_state = seed1)
ADASYN2 = ADASYN(sampling_strategy = 1.0, n_neighbors = 4, random_state = seed1)
...
NCL2 = NeighbourhoodCleaningRule(sampling_strategy = 'auto', n_neighbors = 3, threshold_cleaning = 0.5, n_jobs = -1)
