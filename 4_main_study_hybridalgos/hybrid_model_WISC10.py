...
UBSVM_opt = BalancedBaggingClassifier(base_estimator = SVM, n_estimators = 20, sampling_strategy = 0.8, max_samples = 1.0, random_state = seed_custom, n_jobs = -1)
...
ADA = AdaBoostClassifier(n_estimators = 25, random_state = seed_custom)
...
EASYADA_opt = EasyEnsembleClassifier(base_estimator = ADA, sampling_strategy = 0.9, n_estimators = 10)
EASYADA_performance = algo_CVmetrics(classifier_object = EASYADA_opt, X_train = X_train, Y_train = Y_train)

## long computation time, and even higher for optimization..
...
EASYXGB_opt = EasyEnsembleClassifier(base_estimator = XGB, sampling_strategy = 0.9, n_estimators = 20)
EASYXGB_performance = algo_CVmetrics(classifier_object = EASYXGB_opt, X_train = X_train, Y_train = Y_train)

## marginally better than original EasyEnsemble with ADA
## yields higher CV performance compared to most sampling algorithms
## consistency is higher due to bagging which is important since oversampling algorithms have variance in their performance/outcomes.
