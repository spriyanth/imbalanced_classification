...
remove_n = 170 # we have to delete 120 pos. obs to achieve the 1:5 ratio.
...
## Balanced Accuracy: 0.861 / F2-Score: 0.757 => still works adequately well. even better than on the synthetic dataset, despite the small sample. => good sign.


XGBDT = XGBClassifier(booster = 'gbtree', n_estimators = 100, max_depth = 3, min_child_weight = 1, random_state = seed_custom, early_stopping_rounds = 10)
...
ROS_grid = dict(sampling_strategy = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0])
...
ROSopt_params = dict(sampling_strategy = 0.4, random_state = seed_custom)
BAG_ROSopt_f2 = balanced_fit(X_train, Y_train, BAG, BAG_params, ROS, ROSopt_params) # optmized setup: F2-Score = 0.842 
...
SMOTE_grid = dict(sampling_strategy = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0],
...
SMOTEopt_params = dict(sampling_strategy = 0.4, k_neighbors = 6, random_state = seed_custom) 
BAG_SMOTEopt_f2 = balanced_fit(X_train, Y_train, BAG, BAG_params, SMOTE, SMOTEopt_params) # optmized setup: F2-Score = 0.86 
...
ADASYN_grid = dict(sampling_strategy = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0],
...
ADASYNopt_params = dict(sampling_strategy = 0.3, n_neighbors = 6, random_state = seed_custom)
BAG_ADASYN_opt_f2 = balanced_fit(X_train, Y_train, BAG, BAG_params, ADASYN, ADASYNopt_params) # optmized setup: F2-Score = 0.856
...
RUS_grid = dict(sampling_strategy = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0])
...
RUSopt_params = dict(sampling_strategy = 0.5, random_state = seed_custom) 
BAG_RUSopt_f2 = balanced_fit(X_train, Y_train, BAG, BAG_params, RUS, RUSopt_params) # optmized setup: F2-Score = 0.938 
...
ENNopt_params = dict(sampling_strategy = 'auto', n_neighbors = 7, kind_sel = 'all') 
BAG_ENNopt_f2 = balanced_fit(X_train, Y_train, BAG, BAG_params, ENN, ENNopt_params) # optmized setup: F2-Score = 0.916 
...
NCR_grid = dict(n_neighbors = np.arange(3, 11, 1)) # threshold only necessary if too many observations get deleted. (not the case here)
...
NCRopt_params = dict(sampling_strategy = 'auto', n_neighbors = 7, threshold_cleaning = 0.5, n_jobs = -1)
BAG_NCRopt_f2 = balanced_fit(X_train, Y_train, BAG, BAG_params, NCR, NCRopt_params) # optmized setup: F2-Score = 0.906
...
ROSopt_params2 = dict(sampling_strategy = 0.4, random_state = seed_custom)
XGB_ROSopt_f2 = balanced_fit(X_train, Y_train, XGB, XGB_params, ROS, ROSopt_params2) # optmized setup: F2-Score = 0.829
...
SMOTEopt_params2 = dict(sampling_strategy = 0.4, k_neighbors = 5, random_state = seed_custom) # best 3 candidates have approx. equal stats => choose simplest configuration 
XGB_SMOTEopt_f2 = balanced_fit(X_train, Y_train, XGB, XGB_params, SMOTE, SMOTEopt_params2) # optmized setup: F2-Score = 0.85
...
ADASYN_grid2 = dict(sampling_strategy = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0],
                  n_neighbors = [3, 4, 5, 6, 7, 8, 9, 10])   
...
ADASYNopt_params2 = dict(sampling_strategy = 0.3, n_neighbors = 4, random_state = seed_custom) 
XGB_ADASYN_opt_f2 = balanced_fit(X_train, Y_train, XGB, XGB_params, ADASYN, ADASYNopt_params2) # optmized setup: F2-Score = 0.853
...
RUSopt_params2 = dict(sampling_strategy = 0.6, random_state = seed_custom)
XGB_RUSopt_f2 = balanced_fit(X_train, Y_train, XGB, XGB_params, RUS, RUSopt_params2) # optmized setup: F2-Score = 0.861
...
ENN_grid2 = dict(n_neighbors = np.arange(2, 11, 1))

### GRID SEARCH OPTIMIZATION

XGB_ENN_scoreboard = []

for neighbors in ENN_grid2['n_neighbors']:
    ENN_params_new = dict(sampling_strategy = 'auto', n_neighbors = neighbors, kind_sel = 'all')
...
ENNopt_params2 = dict(sampling_strategy = 'auto', n_neighbors = 2, kind_sel = 'all') 
XGB_ENNopt_f2 = balanced_fit(X_train, Y_train, XGB, XGB_params, ENN, ENNopt_params2) # optmized setup: F2-Score = 0.86
...
NCRopt_params2 = dict(sampling_strategy = 'auto', n_neighbors = 4, threshold_cleaning = 0.5, n_jobs = -1) # config with 7,9,10 neighbors is marginally better. choose more conservative setup and avoid spurious results.
XGB_NCRopt_f2 = balanced_fit(X_train, Y_train, XGB, XGB_params, NCR, NCRopt_params2) # optmized setup: F2-Score = 0.83