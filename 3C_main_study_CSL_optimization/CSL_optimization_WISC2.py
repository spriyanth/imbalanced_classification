...
BAGSVM_opt = BaggingClassifier(base_estimator = SVM, n_estimators = 100, max_samples = 0.8, bootstrap = True, n_jobs = -1, random_state = seed_custom)
...
BAGWSVM_metrics = algo_CVmetrics(classifier_object = BAGWSVM, X_train = X_train, Y_train = Y_train) # BAcc = 0.947, F2-Score = 0.928 => minimal improvement (performance is already very high, it's hard to improve the model further w/o losing the ability to generalize on unseen data)

############### BAGGING + WEIGHTED SVM (WITH OPTIMIZED WEIGHTS) ###############

weight_range = [{0:1,1:1}, {0:1,1:1.25}, {0:1,1:1.5}, {0:1,1:1.75}, {0:1,1:2}, {0:1,1:2.25}, {0:1,1:2.5}, {0:1,1:2.75}, {0:1,1:3}, 
...
BAGWSVM_opt_metrics = algo_CVmetrics(classifier_object = BAGWSVM_opt, X_train = X_train, Y_train = Y_train) # BAcc = 0.948, F2-Score = 0.933 => marginally better than heuristic setup (neglectable difference..)
...
WXGB_metrics = algo_CVmetrics(classifier_object = WXGB, X_train = X_train, Y_train = Y_train) # BAcc = 0.94, F2-Score = 0.93 => marginally better with heuristic weight 

############### WEIGHTED XGB (WITH OPTIMIZED WEIGHTS) ###############

scale = np.arange(1, 3.5, 0.5) # wider grid is prone to overfitting => always chooses max value of the grid..
...
WXGB_opt = XGBClassifier(booster = 'gbtree', n_estimators = 200, scale_pos_weight = 2, max_depth = 2, min_child_weight = 1, random_state = seed_custom, early_stopping_rounds = 10)
WXGB_opt_metrics = algo_CVmetrics(classifier_object = WXGB_opt, X_train = X_train, Y_train = Y_train) 